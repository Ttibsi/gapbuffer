    ...
    private:
        ...
        class IteratorTemplate {
            public:
                ...
                using value_type = std::conditional<is_const, const Gapvector::value_type, Gapvector::value_type>::type;
                ...
                explicit IteratorTemplate(gapvector_pointer self, PointerType input_ptr): gv(self), ptr(input_ptr) {}

                IteratorTemplate& operator++() {
                    ptr += 1;
                    if (ptr == gv->gapStart) { ptr = gv->gapEnd; }
                    return *this;
                }
                ...
                IteratorTemplate& operator--() {
                    ptr -= 1;
                    if (ptr == gv->gapEnd) { ptr = gv->gapStart - 1; }
                    return *this;
                }
    ...
    public:
       ...
       [[nodiscard]] constexpr reference front() {
        ...
        [[nodiscard]] std::string line(size_type pos) const {
            ...
            if (empty()) { throw std::runtime_error("Cannot pull line from empty gapvector"); }
        ...
        const_reverse_iterator rend() const noexcept {
            return const_reverse_iterator(const_iterator(this, (gapStart == bufferStart) ? gapEnd : bufferStart));
        }
        const_reverse_iterator crbegin() const noexcept { return const_reverse_iterator(this->cbegin()); }
        ...
        constexpr void push_back(const T& value) {
            if (gapStart == gapEnd) { resize(capacity() * 2); }
            *gapStart = value;
            gapStart++;
        }
        ...
        constexpr void resize(const size_type count) {
            ...
            std::uninitialized_copy_n(gapEnd, suffix_size, new_mem + prefix_size + (count - capacity()));
